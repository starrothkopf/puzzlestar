...
export default function StatTransfer({closeModal}) {
    ...
    const handleTransfer = (async () => {
        ...
        try {
            ...
            await patchData('wordle_wins', numWins); // TODO: calculate number of wins from win percent and num played
            await patchData(`wordle_guesses.1`, guess1);
            await patchData(`wordle_guesses.2`, guess2);
            await patchData(`wordle_guesses.3`, guess3);
            await patchData(`wordle_guesses.4`, guess4);
            await patchData(`wordle_guesses.5`, guess5);
            await patchData(`wordle_guesses.6`, guess6);
