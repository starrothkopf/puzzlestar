...
const Wordle = ({solution}) => {
    ...
    const { currentGuess, handleKeyup, guesses, loadGameState.isCorrect, loadGameState.turn, usedKeys } = useWordle(solution);
    const { currentUser } = useContext(AuthContext);
    const [showModal, setShowModal] = useState(false)
    const [countdown, setCountdown] = useState({ hours: 0, minutes: 0, seconds: 0 });

    useEffect(() => {
      saveGameState({ turn, isCorrect });
    }, [turn, isCorrect]);
...
// Helper function to load game state from local storage
const loadGameState = () => {
  const gameStateJSON = localStorage.getItem('wordleGameState');
  return gameStateJSON ? JSON.parse(gameStateJSON) : null;
};