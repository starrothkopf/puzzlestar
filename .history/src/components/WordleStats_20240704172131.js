...
export default function WordleStats({ id }) {
    ...
    return (
        <div className="stats">
            ...
            <div className="bars">
                <div className="bar" style={{ width: `${getBarWidth(currentUser["won_in_2"])}%` }}>2: {currentUser["won_in_1"]}</div>
                <div className="bar" style={{ width: `${getBarWidth(currentUser["won_in_3"])}%` }}>3: {currentUser["won_in_1"]}</div>
                <div className="bar" style={{ width: `${getBarWidth(currentUser["won_in_4"])}%` }}>4: {currentUser["won_in_1"]}</div>
                <div className="bar" style={{ width: `${getBarWidth(currentUser["won_in_5"])}%` }}>5: {currentUser["won_in_1"]}</div>
                <div className="bar" style={{ width: `${getBarWidth(currentUser["won_in_6"])}%` }}>6: {currentUser["won_in_1"]}</div>
            </div>
        </div>
    )
}