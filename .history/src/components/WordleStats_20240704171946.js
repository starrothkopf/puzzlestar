...
export default function WordleStats({ id }) {
    ...
    const getAverage = () => {
        ...
        const totalGuesses = currentUser["won_in_1"] * 1 + currentUser["won_in_2"] * 2 + currentUser["won_in_3"] * 3 + currentUser["won_in_4"] * 4 + currentUser["won_in_5"] * 5 + currentUser["won_in_2"] * 6;
        if (totalWins === 0) {
            return "0";
        }
