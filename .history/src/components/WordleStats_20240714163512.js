...
export default function WordleStats({ id }) {
    ...
    const getBarWidth = (value) => {
        ...
        const maxWins = Math.max(currentUser.wordle_guesses[turn], currentUser["won_in_2"], currentUser["won_in_3"], currentUser["won_in_4"], currentUser["won_in_5"], currentUser["won_in_6"]);
    ...
    const getWinPercentage = () => {
        if (currentUser["num_played_wordle"] === 0) {
            return 100;
        }
        return ((currentUser.num_wins_wordle / currentUser.num_played_wordle) * 100).toFixed(1);
    }
