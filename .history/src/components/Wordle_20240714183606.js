...
const Wordle = ({solution}) => {
  ...
  const handleGameEnd = useCallback(async (playTime) => {
    ...
    let updatedUser = await patchData('wordle_lastPlayDate', playTime);
    updatedUser = await patchData('wordle_plays', updatedUser.wordle_plays + 1);
    
    if (isCorrect) {
        updatedUser = await patchData('wordle_wins', updatedUser.wordle_wins + 1);
        updatedUser = await patchData(`wordle_guesses.${turn}`, updatedUser.wordle_guesses[turn] + 1);
    }
    
    updateCurrentUser(updatedUser);
    setPlayedToday(true);
  }, [currentUser, updateCurrentUser, handleKeyup, isCorrect, turn, patchData]);
